...
class CustomDataset(Dataset):
    def __init__(self, dataset='ps2', state='train'):  
        ...
        self.dataset_config['grid_height'] = int(self.dataset_config['img_height'] / MODEL_CONFIG['model_stride'])
        self.dataset_config['grid_width'] = int(self.dataset_config['img_width'] / MODEL_CONFIG['model_stride'])
    ...
    def generate_slot_labels(self, slots, state):
        if state == 'train' or state == 'val':
            labels = np.zeros((self.dataset_config['grid_height'], self.dataset_config['grid_width'], MODEL_CONFIG['label_channel']))
            ...
            for slot in slots:
                ...
                # Fill data for cells that are inside a parking slot
                for xid in range(self.dataset_config['grid_height']):
                    for yid in range(self.dataset_config['grid_width']):
                        check = inside_slot((yid, xid), slot, self.dataset_config['img_height'], self.dataset_config['img_width'], self.dataset_config['l_mean'])
                        if check == 1:
                            # Posibility that the cell in inside a parking slot
                            labels[xid, yid, 0] = 1
                            # Relative position from the cell center to the slot's junction
                            labels[xid, yid, 1:3] = (junction1 - np.array([yid + 0.5, xid + 0.5]) * MODEL_CONFIG['model_stride']) / self.dataset_config['l_max']
                            labels[xid, yid, 3:5] = (junction2 - np.array([yid + 0.5, xid + 0.5]) * MODEL_CONFIG['model_stride']) / self.dataset_config['l_max']
                            # Type of the parking slot
                            labels[xid, yid, 5 + slot_type] = 1
                            # Occupancy of the parking slot
                            labels[xid, yid, 8] = slot_occ
                
                # Fill data for cells that contain junction
                for junction, ori in zip([junction1, junction2], [ori1, ori2]):
                    yid = int((junction[0]) / MODEL_CONFIG['model_stride'])
                    xid = int((junction[1]) / MODEL_CONFIG['model_stride'])

                    if yid == self.dataset_config['grid_width']:
                        yid -= 1
                    if xid == self.dataset_config['grid_height']:
                        xid -= 1

                    # Posibility that the cell contains a junction:
                    labels[xid, yid, 9] = 1
                    # Junction location (relative position to cell center, value from -0.5 to 0.5)
                    labels[xid, yid, 10:12] = junction / MODEL_CONFIG['model_stride'] - np.array([yid + 0.5, xid + 0.5])
                    # Junction orientation
                    labels[xid, yid, 12:14] = ori
    ...
    def visualize_label(self, imgs, labels, index, mode=0):
        ...
        labels = labels[index].detach().cpu().numpy()
        
        if self.state == 'train':
            if mode == 0: # global information visualization
                labels = labels.reshape([-1, MODEL_CONFIG['label_channel']])
                for id, item in enumerate(labels):
                    if item[0] != 0:
                        ...
                        center = np.array([(yid + 0.5), (xid + 0.5)]) * MODEL_CONFIG['model_stride']

                        junc1 = center + item[1:3] * self.dataset_config['l_max']
                        junc2 = center + item[3:5] * self.dataset_config['l_max']
                        
                        slot_type = np.argmax(item[5:8])
                        slot_occ = item[8]

                        color = self.dataset_config['color_dict'][slot_type]
                        if slot_occ == 0.0:
                            plt.plot(center[0], center[1], 'o', color=color, markersize=12)
                            plt.arrow(center[0], center[1], (junc1[0] - center[0]), (junc1[1] - center[1]), head_width=6, ls='-', color=color, linewidth=3)
                            plt.arrow(center[0], center[1], (junc2[0] - center[0]), (junc2[1] - center[1]), head_width=6, ls='-', color=color, linewidth=3)
                        else:
                            plt.plot(center[0], center[1], 'o', color=color, markersize=12)
                            plt.arrow(center[0], center[1], (junc1[0] - center[0]), (junc1[1] - center[1]), head_width=6, ls='--', color=color, linewidth=3)
                            plt.arrow(center[0], center[1], (junc2[0] - center[0]), (junc2[1] - center[1]), head_width=6, ls='--', color=color, linewidth=3)

            if mode == 1: # local information visualization
                labels = labels.reshape([-1, MODEL_CONFIG['label_channel']])
                for id, item in enumerate(labels):
                    if item[9] != 0:
                        xid, yid = np.unravel_index(id, (self.dataset_config['grid_height'], self.dataset_config['grid_width']))
                        
                        junction = (np.array([(yid + 0.5), (xid + 0.5)]) + item[10:12]) * MODEL_CONFIG['model_stride']
                        ori = item[12:14]

                        plt.plot(junction[0], junction[1], 'o', color='r', markersize=12)
                        plt.arrow(junction[0], junction[1], ori[0]*25, ori[1]*25, head_width=6, ls='-', color='r', linewidth=3)
