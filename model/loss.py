# ------------------------------------------------------------- #
# End-to-end one-stage with global and local information        #
# Loss funtions                                                 #
#                                                               #
# ------------------------------------------------------------- #
# IVPG Lab                                                      #
# Author: Quang Huy Bui                                         #
# Modified date: August 2025                                    #
# ------------------------------------------------------------- #


import torch
import torch.nn.functional as F
from torch.utils.tensorboard import SummaryWriter
from torchmetrics.aggregation import MeanMetric

from config.config import LOSS_CONFIG

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


class Metric():
    def __init__(self, log_folder):
        self.g_inside_slot = MeanMetric().to(device)
        self.g_junc1 = MeanMetric().to(device)
        self.g_junc2 = MeanMetric().to(device)
        self.g_slot_type = MeanMetric().to(device)
        self.g_slot_occ = MeanMetric().to(device)
        self.l_junc_conf = MeanMetric().to(device)
        self.l_junc_loc = MeanMetric().to(device)
        self.l_junc_ori = MeanMetric().to(device)
        self.total_loss = MeanMetric().to(device)

        self.writer = SummaryWriter(log_folder)
    
    def reset(self):
        self.g_inside_slot.reset()
        self.g_slot_type.reset()
        self.g_slot_occ.reset()
        self.g_junc1.reset()
        self.g_junc2.reset()
        self.l_junc_conf.reset()
        self.l_junc_loc.reset()
        self.l_junc_ori.reset()
        self.total_loss.reset()

    def update(self, losses):
        self.g_inside_slot.update(losses[0])
        self.g_junc1.update(losses[1])
        self.g_junc2.update(losses[2])
        self.g_slot_type.update(losses[3])
        self.g_slot_occ.update(losses[4])
        self.l_junc_conf.update(losses[5])
        self.l_junc_loc.update(losses[6])
        self.l_junc_ori.update(losses[7])
        self.total_loss.update(losses[8])
    
    def write(self, epoch, lr, state):
        if state == 'train':
            self.writer.add_scalar('lr', lr, global_step=epoch)

        self.writer.add_scalar(state + '_loss/inside_slot_loss', self.g_inside_slot.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/junc1_loss', self.g_junc1.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/junc2_loss', self.g_junc2.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/slot_type_loss', self.g_slot_type.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/slot_occ_loss', self.g_slot_occ.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/junc_conf_loss', self.l_junc_conf.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/junc_loc_loss', self.l_junc_loc.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/junc_ori_loss', self.l_junc_ori.compute(), global_step=epoch)
        self.writer.add_scalar(state + '_loss/total_loss', self.total_loss.compute(), global_step=epoch)

        self.writer.flush()

    def compute(self):
        return [self.g_inside_slot.compute(),
                self.g_junc1.compute(),
                self.g_junc2.compute(),
                self.g_slot_type.compute(),
                self.g_slot_occ.compute(),
                self.l_junc_conf.compute(),
                self.l_junc_loc.compute(),
                self.l_junc_ori.compute(),
                self.total_loss.compute()]
        





class Losses():
    def __init__(self, dataset_config):
        self.dataset_config = dataset_config


    def conf_loss(self, pred, true, weight):
        loss = torch.square(pred - true)
        loss = (true == 0) * loss * weight[0] + (true == 1) * loss * weight[1]
        # loss = torch.sum(loss, dim=[1, 2])
        loss = torch.mean(loss) 

        return loss
    
    def loc_loss(self, pred, true, mask):
        loss = torch.square(pred - true)
        loss = mask * torch.sum(loss, -1)
        loss = torch.sum(loss) / (torch.sum(mask) + 1e-9)

        return loss
    
    def ori_loss(self, pred, true, mask):
        pred = F.normalize(pred, dim=-1)
        true = F.normalize(true, dim=-1)

        loss = torch.square(pred - true)
        loss = mask * torch.sum(loss, -1)
        loss = torch.sum(loss) / (torch.sum(mask) + 1e-9)

        return loss
    
    def type_loss(self, pred, true, mask, weight):
        pred = pred.permute(0, 3, 1, 2)
        true = torch.argmax(true, dim=-1).to(torch.long)#.permute(0, 3, 1, 2)
        class_weights = torch.tensor(weight).to(pred.device)

        loss_fn = torch.nn.CrossEntropyLoss(weight=class_weights, reduction='none')
        loss = loss_fn(pred, true)
        loss = mask * loss
        loss = torch.sum(loss) / (torch.sum(mask) + 1e-9)

        return loss
    
    def occ_loss(self, pred, true, mask, weight):
        loss = torch.square(pred - true)
        loss = mask * loss
        loss = (true == 0) * loss * weight[0] + (true == 1) * loss * weight[1]
        loss = torch.sum(loss) / (torch.sum(mask) + 1e-9)

        return loss



    def compute(self, output, labels):
        inside_slot = self.conf_loss(output[:, :, :, 0], labels[:, :, :, 0], self.dataset_config['inslot_weights']) * LOSS_CONFIG['inside_slot']
        junc1 = self.loc_loss(output[:, :, :, 1:3], labels[:, :, :, 1:3], labels[:, :, :, 0] == 1) * LOSS_CONFIG['junc']
        junc2 = self.loc_loss(output[:, :, :, 3:5], labels[:, :, :, 3:5], labels[:, :, :, 0] == 1) * LOSS_CONFIG['junc']
        slot_type = self.type_loss(output[:, :, :, 5:8], labels[:, :, :, 5:8], labels[:, :, :, 0] == 1, self.dataset_config['class_weights']) * LOSS_CONFIG['slot_type']
        slot_occ = self.occ_loss(output[:, :, :, 8], labels[:, :, :, 8], labels[:, :, :, 0] == 1, self.dataset_config['occupancy_weights']) * LOSS_CONFIG['slot_occ']

        junc_conf = self.conf_loss(output[:, :, :, 9], labels[:, :, :, 9], self.dataset_config['junc_weights']) * LOSS_CONFIG['junc_conf']
        junc_loc = self.loc_loss(output[:, :, :, 10:12] - 0.5, labels[:, :, :, 10:12], labels[:, :, :, 9] == 1) * LOSS_CONFIG['junc_loc']
        junc_ori = self.ori_loss(output[:, :, :, 12:14], labels[:, :, :, 12:14], labels[:, :, :, 9] == 1) * LOSS_CONFIG['junc_ori']

        total = inside_slot + junc1 + junc2 + slot_type + slot_occ + junc_conf + junc_loc + junc_ori 

        return [inside_slot, junc1, junc2, slot_type, slot_occ, junc_conf, junc_loc, junc_ori, total]


