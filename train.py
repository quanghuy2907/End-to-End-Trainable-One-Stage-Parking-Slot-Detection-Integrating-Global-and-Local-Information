...
model = Model(d_model=MODEL_CONFIG['d_model'])
...
for epoch in range(TRAIN_CONFIG['epoch_num']):
    ...
    for i, data in enumerate(train_loader):
        ...
        s = metric.compute()
        sys.stdout.write('\r' + f'step: {i}/{len(train_loader)} || inslot:{s[0]:.4f} | junc1:{s[1]:.4f} | junc2:{s[2]:.4f} | type:{s[3]:.4f} | occ: {s[4]:.4f} | junc_conf:{s[5]:.4f} | junc_loc:{s[6]:.4f} | junc_ori:{s[7]:.4f} || train_loss:{s[8]:.4f}')
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader):
            ...
            s = metric.compute()
            sys.stdout.write('\r' + f'step: {i}/{len(train_loader)} || inslot:{s[0]:.4f} | junc1:{s[1]:.4f} | junc2:{s[2]:.4f} | type:{s[3]:.4f} | occ:{s[4]:.4f} | junc_conf:{s[5]:.4f} | junc_loc:{s[6]:.4f} | junc_ori:{s[7]:.4f} || val_loss:{s[8]:.4f}')
